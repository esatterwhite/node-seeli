...
const {default: width} = require('string-width')
...
function options(command, plain) {
  ...
  for (const [flag, config] of Object.entries(flags)) {
    ...
    if (config.required_with) {
      required_with_ui.div({
        text: colorize(`* ${flag}, ${config.required_with.join(', ')}`)
      , padding: [0, 0, 0, 2]
      })
    }
    if (config.required_without) {
      required_without_ui.div({
        text: colorize(`* ${flag}, ${config.required_without.join(', ')}`)
      , padding: [0, 0, 0, 2]
      })
    }
  }
  ...
  if (requires_one) {
    requires_one_ui.div({
      text: colorize(`* ${requires_one.join(', ')}`)
    , padding: [0, 0, 0, 2]
    })
  }
  ...
  ui.div({
    text: [
      `${colorize('<...>')}: input type`
    , `${colorize('*')}: repeatable flags`
    , `${colorize('[...]')}: default values`
    ].join(' | ')
  , padding: [2, 0, 0, 2]
  })
