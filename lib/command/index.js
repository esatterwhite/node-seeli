...
const Equirer = require('enquirer')
...
class Command extends Registry {
  constructor(...options) {
    ...
    // Allow injecting a custom enquirer instance for testing
    if (this.options.enquirer) {
      this[kPrompt] = this.options.enquirer
    }
  ...
  async interactive(cmd) {
    ...
    while (flags.length) {
      ...
      if (typeof current.validate === 'function') {
        const is_valid = current.validate.call(this, {...this.parsed, ...answers})
   ...
   **/
  prompt(opts) {
    const enquirer = this[kPrompt]
    return enquirer.prompt(opts)
  }
  ...
  async ask(name, opts) {
    ...
    while (true) {
      const answer = await this[kPrompt].prompt(question)
