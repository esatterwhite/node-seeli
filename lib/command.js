...
class Command extends events.EventEmitter {
	...
	interactive( cmd, done ){
		...
		for( let flag of flags ) {
			let current = that.options.flags[flag]
			  , arg
			  ;

			if( stop_flags.indexOf( flag ) != -1 ){
				continue;
			}

			arg = {
				type: current.type === Boolean ? 'confirm' : current.mask ? 'password' : 'input'
				,name: flag
		...
		inquirer
			...
			.then(function( answers ){
                ...
                that.validate( );
				that.dispatch( );
				return that.options.run.call(that, cmd, {}, done );
			});
	}
	...
	validate( cmd ){
		...
		Object
			...
			.forEach(function( flag ){
				...
				if(isValid === false ){
					return that.emit('error', new exceptions.InvalidFieldException( util.format("%s failed validation.", flag) ) );
				}

				if( typeof isValid == 'string'){
					return that.emit('error', new exceptions.InvalidFieldException( util.format("%s - %s", flag, isValid ) ) );
				}
			});
	}
