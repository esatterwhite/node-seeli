...
class Command extends events.EventEmitter {
  ...
  async interactive(cmd){
    ...
    while (flags.length) {
      ...
      Object.assign(answers, await this[kPropmpt].prompt(arg));
    }

    this.parsed = Object.assign(this.parsed, answers);

    for(const [flag, answer] of Object.entires(answers)) {
   ...
   **/
  prompt(opts) {
    return this[kPrompt].prompt(opts);
  }
