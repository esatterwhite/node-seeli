...
var  Prime        = require( 'prime' )              // this is prime
   ...
   , stop_flags   = ['help', 'interactive', 'skip']
...
Command = Prime(/** @lends module:NAME.Command.prototype */{
	...
	, interactive: function interactive( cmd, done ){
		...
		Object
			...
			.filter( function( flag ){
				return stop_flags.indexOf( flag ) >= 0;
			})
			.forEach(function( flag ){
				...
				arg.when     = current.when     || undefined;
				arg.validate = current.validate || undefined;
				arg.filter   = current.filter   || undefined;
	...
	, validate: function validate( ){
		...
		this._required.forEach(function( flag ){
			var cfg = this.options.flags[flag];
			var validator = cfg.validate;
			
			if( this.parsed[flag] === UNDEF ){
				this.emit('error', new exceptions.RequiredFieldError( flag ) );
			}

			var isValid = validator ? validator( this.parsed ) : true

			if(isValid === false ){
				return this.emit('error', new exceptions.InvalidFieldException( util.format("%s failed validation.") ) )
			}

			if( typeof isValid == 'string'){
				return this.emit('error', new exceptions.InvalidFieldException( isValid ) )
			}
